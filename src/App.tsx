...
function App() {
  ...
  return (
    <>
      {/* Audio element MUST be constantly in the DOM so the ref is always valid and available when the button is clicked */}
      <audio 
        ref={audioRef}
        src="song.mp3" 
        preload="auto"
      ...
      {!accessGranted ? (
        <div
          className="access-denied bg-dark text-white d-flex flex-column justify-content-center align-items-center vh-100 text-center"
          dir="rtl"
        >
          <h1 className="fw-light display-4 mb-4">שאלה מיוחדת...</h1>
          <p className="lead text-muted px-4">אנא ודאי שיש לך את הקישור הנכון.</p>
        </div>
      ) : !started ? (
        <div
          className="start-screen bg-dark text-white d-flex flex-column justify-content-center align-items-center vh-100 text-center"
          dir="rtl"
        >
          <h1 className="fw-light display-3 mb-5 px-4 animate-fade-in">מוכנה?</h1>
          <button
        ...
        <div className="gallery-container bg-dark vh-100 vw-100 overflow-hidden position-relative">
          {!isFinished ? (
            photos.map((photoUrl, index) => (
              ...
              >
                ...
                <img
                  src={photoUrl}
                  alt="Memory"
                  className="slide-img shadow-lg rounded"
                  referrerPolicy="no-referrer"
                />
              </div>
            ))
            ...
            <div
              className="final-screen d-flex flex-column justify-content-center align-items-center w-100 h-100 text-white text-center"
              dir="rtl"
            >
